...
METHOD = 'employee_get_by_code'
REQUEST_MODEL = 'EmployeeGetByCodeRequest'
FIELD_NAMES = ['EmployeeCode']
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'employee_get_by_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
EMPLOYEE_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if EMPLOYEE_CODE_OVERRIDE:
        return {"EmployeeCode": EMPLOYEE_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_EMPLOYEE_CODE", "").strip()
    if env_value:
        return {"EmployeeCode": env_value}
...
class TestEmployeeGetByCode(unittest.TestCase):
    ...
    def test_employee_get_by_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--employee-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        EMPLOYEE_CODE_OVERRIDE = args.field_value.strip()
