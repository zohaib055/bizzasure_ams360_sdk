...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'broker_get_by_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
BROKER_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if BROKER_CODE_OVERRIDE:
        return {"BrokerCode": BROKER_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_BROKER_CODE", "").strip()
    if env_value:
        return {"BrokerCode": env_value}
...
class TestBrokerGetByCode(unittest.TestCase):
    ...
    def test_broker_get_by_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--broker-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        BROKER_CODE_OVERRIDE = args.field_value.strip()
