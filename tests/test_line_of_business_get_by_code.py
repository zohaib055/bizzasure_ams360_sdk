...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'line_of_business_get_by_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
LINE_OF_BUSINESS_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if LINE_OF_BUSINESS_CODE_OVERRIDE:
        return {"LineOfBusinessCode": LINE_OF_BUSINESS_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_LINE_OF_BUSINESS_CODE", "").strip()
    if env_value:
        return {"LineOfBusinessCode": env_value}
...
class TestLineOfBusinessGetByCode(unittest.TestCase):
    ...
    def test_line_of_business_get_by_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--line-of-business-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        LINE_OF_BUSINESS_CODE_OVERRIDE = args.field_value.strip()
