...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_group_get_list_by_name_prefix.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_GROUP_NAME_PREFIX_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_GROUP_NAME_PREFIX_OVERRIDE:
        return {"GLGroupNamePrefix": GL_GROUP_NAME_PREFIX_OVERRIDE}

    env_value = os.getenv("AMS360_GL_GROUP_NAME_PREFIX", "").strip()
    if env_value:
        return {"GLGroupNamePrefix": env_value}
...
class TestGlGroupGetListByNamePrefix(unittest.TestCase):
    ...
    def test_gl_group_get_list_by_name_prefix(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-group-name-prefix", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_GROUP_NAME_PREFIX_OVERRIDE = args.field_value.strip()
