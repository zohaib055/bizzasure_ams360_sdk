...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'employee_get_by_short_name.json'
LOG_PATH = ROOT_DIR / "log.txt"
EMPLOYEE_SHORT_NAME_OVERRIDE = None
...
def _load_payload() -> dict:
    if EMPLOYEE_SHORT_NAME_OVERRIDE:
        return {"EmployeeShortName": EMPLOYEE_SHORT_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_EMPLOYEE_SHORT_NAME", "").strip()
    if env_value:
        return {"EmployeeShortName": env_value}
...
class TestEmployeeGetByShortName(unittest.TestCase):
    ...
    def test_employee_get_by_short_name(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--employee-short-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        EMPLOYEE_SHORT_NAME_OVERRIDE = args.field_value.strip()
