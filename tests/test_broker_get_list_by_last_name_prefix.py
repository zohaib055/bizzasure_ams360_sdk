...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'broker_get_list_by_last_name_prefix.json'
LOG_PATH = ROOT_DIR / "log.txt"
BROKER_LAST_NAME_PREFIX_OVERRIDE = None
...
def _load_payload() -> dict:
    if BROKER_LAST_NAME_PREFIX_OVERRIDE:
        return {"BrokerLastNamePrefix": BROKER_LAST_NAME_PREFIX_OVERRIDE}

    env_value = os.getenv("AMS360_BROKER_LAST_NAME_PREFIX", "").strip()
    if env_value:
        return {"BrokerLastNamePrefix": env_value}
...
class TestBrokerGetListByLastNamePrefix(unittest.TestCase):
    ...
    def test_broker_get_list_by_last_name_prefix(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--broker-last-name-prefix", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        BROKER_LAST_NAME_PREFIX_OVERRIDE = args.field_value.strip()
