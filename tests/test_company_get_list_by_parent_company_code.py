...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'company_get_list_by_parent_company_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
PARENT_COMPANY_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if PARENT_COMPANY_CODE_OVERRIDE:
        return {"ParentCompanyCode": PARENT_COMPANY_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_PARENT_COMPANY_CODE", "").strip()
    if env_value:
        return {"ParentCompanyCode": env_value}
...
class TestCompanyGetListByParentCompanyCode(unittest.TestCase):
    ...
    def test_company_get_list_by_parent_company_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--parent-company-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        PARENT_COMPANY_CODE_OVERRIDE = args.field_value.strip()
