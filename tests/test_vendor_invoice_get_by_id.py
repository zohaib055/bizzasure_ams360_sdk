...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'vendor_invoice_get_by_id.json'
LOG_PATH = ROOT_DIR / "log.txt"
VENDOR_INVOICE_ID_OVERRIDE = None
...
def _load_payload() -> dict:
    if VENDOR_INVOICE_ID_OVERRIDE:
        return {"VendorInvoiceId": VENDOR_INVOICE_ID_OVERRIDE}

    env_value = os.getenv("AMS360_VENDOR_INVOICE_ID", "").strip()
    if env_value:
        return {"VendorInvoiceId": env_value}
...
class TestVendorInvoiceGetById(unittest.TestCase):
    ...
    def test_vendor_invoice_get_by_id(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vendor-invoice-id", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        VENDOR_INVOICE_ID_OVERRIDE = args.field_value.strip()
