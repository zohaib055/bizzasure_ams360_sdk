...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'vendor_get_list_by_is_company.json'
LOG_PATH = ROOT_DIR / "log.txt"
IS_COMPANY_OVERRIDE = None
...
def _load_payload() -> dict:
    if IS_COMPANY_OVERRIDE:
        return {"IsCompany": IS_COMPANY_OVERRIDE}

    env_value = os.getenv("AMS360_IS_COMPANY", "").strip()
    if env_value:
        return {"IsCompany": env_value}
...
class TestVendorGetListByIsCompany(unittest.TestCase):
    ...
    def test_vendor_get_list_by_is_company(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is-company", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        IS_COMPANY_OVERRIDE = args.field_value.strip()
