...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'company_get_list_by_name_prefix.json'
LOG_PATH = ROOT_DIR / "log.txt"
COMPANY_NAME_PREFIX_OVERRIDE = None
...
def _load_payload() -> dict:
    if COMPANY_NAME_PREFIX_OVERRIDE:
        return {"CompanyNamePrefix": COMPANY_NAME_PREFIX_OVERRIDE}

    env_value = os.getenv("AMS360_COMPANY_NAME_PREFIX", "").strip()
    if env_value:
        return {"CompanyNamePrefix": env_value}
...
class TestCompanyGetListByNamePrefix(unittest.TestCase):
    ...
    def test_company_get_list_by_name_prefix(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--company-name-prefix", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        COMPANY_NAME_PREFIX_OVERRIDE = args.field_value.strip()
