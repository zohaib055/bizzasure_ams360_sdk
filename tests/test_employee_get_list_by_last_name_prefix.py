...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'employee_get_list_by_last_name_prefix.json'
LOG_PATH = ROOT_DIR / "log.txt"
EMPLOYEE_LAST_NAME_PREFIX_OVERRIDE = None
...
def _load_payload() -> dict:
    if EMPLOYEE_LAST_NAME_PREFIX_OVERRIDE:
        return {"EmployeeLastNamePrefix": EMPLOYEE_LAST_NAME_PREFIX_OVERRIDE}

    env_value = os.getenv("AMS360_EMPLOYEE_LAST_NAME_PREFIX", "").strip()
    if env_value:
        return {"EmployeeLastNamePrefix": env_value}
...
class TestEmployeeGetListByLastNamePrefix(unittest.TestCase):
    ...
    def test_employee_get_list_by_last_name_prefix(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--employee-last-name-prefix", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        EMPLOYEE_LAST_NAME_PREFIX_OVERRIDE = args.field_value.strip()
