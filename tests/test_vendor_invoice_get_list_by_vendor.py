...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'vendor_invoice_get_list_by_vendor.json'
LOG_PATH = ROOT_DIR / "log.txt"
VENDOR_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if VENDOR_CODE_OVERRIDE:
        return {"VendorCode": VENDOR_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_VENDOR_CODE", "").strip()
    if env_value:
        return {"VendorCode": env_value}
...
class TestVendorInvoiceGetListByVendor(unittest.TestCase):
    ...
    def test_vendor_invoice_get_list_by_vendor(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vendor-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        VENDOR_CODE_OVERRIDE = args.field_value.strip()
