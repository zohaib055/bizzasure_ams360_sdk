...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'customer_service_personnel_get_list.json'
...
def _load_payload() -> dict:
    ...
    env_value = os.getenv("AMS360_CUSTOMER_ID", "").strip()
    if env_value:
        return {"CustomerId": env_value}
...
class TestCustomerServicePersonnelGetList(unittest.TestCase):
    ...
    def test_customer_service_personnel_get_list(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--customer-id", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        CUSTOMER_ID_OVERRIDE = args.field_value.strip()
