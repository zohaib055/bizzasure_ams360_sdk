...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_division_get_by_short_name.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_DIVISION_SHORT_NAME_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_DIVISION_SHORT_NAME_OVERRIDE:
        return {"GLDivisionShortName": GL_DIVISION_SHORT_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_GL_DIVISION_SHORT_NAME", "").strip()
    if env_value:
        return {"GLDivisionShortName": env_value}
...
class TestGlDivisionGetByShortName(unittest.TestCase):
    ...
    def test_gl_division_get_by_short_name(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-division-short-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_DIVISION_SHORT_NAME_OVERRIDE = args.field_value.strip()
