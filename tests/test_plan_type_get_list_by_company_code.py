...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'plan_type_get_list_by_company_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
COMPANY_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if COMPANY_CODE_OVERRIDE:
        return {"CompanyCode": COMPANY_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_COMPANY_CODE", "").strip()
    if env_value:
        return {"CompanyCode": env_value}
...
class TestPlanTypeGetListByCompanyCode(unittest.TestCase):
    ...
    def test_plan_type_get_list_by_company_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--company-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        COMPANY_CODE_OVERRIDE = args.field_value.strip()
