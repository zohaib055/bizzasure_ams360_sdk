...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'customer_suspense_get_by_suspense_id.json'
LOG_PATH = ROOT_DIR / "log.txt"
SUSPENSE_ID_OVERRIDE = None
...
def _load_payload() -> dict:
    if SUSPENSE_ID_OVERRIDE:
        return {"SuspenseId": SUSPENSE_ID_OVERRIDE}

    env_value = os.getenv("AMS360_SUSPENSE_ID", "").strip()
    if env_value:
        return {"SuspenseId": env_value}
...
class TestCustomerSuspenseGetBySuspenseId(unittest.TestCase):
    ...
    def test_customer_suspense_get_by_suspense_id(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--suspense-id", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        SUSPENSE_ID_OVERRIDE = args.field_value.strip()
