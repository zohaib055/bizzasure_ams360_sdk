...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_branch_get_list_by_name_prefix.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_BRANCH_NAME_PREFIX_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_BRANCH_NAME_PREFIX_OVERRIDE:
        return {"GLBranchNamePrefix": GL_BRANCH_NAME_PREFIX_OVERRIDE}

    env_value = os.getenv("AMS360_GL_BRANCH_NAME_PREFIX", "").strip()
    if env_value:
        return {"GLBranchNamePrefix": env_value}
...
class TestGlBranchGetListByNamePrefix(unittest.TestCase):
    ...
    def test_gl_branch_get_list_by_name_prefix(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-branch-name-prefix", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_BRANCH_NAME_PREFIX_OVERRIDE = args.field_value.strip()
