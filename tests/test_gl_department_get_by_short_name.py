...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_department_get_by_short_name.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_DEPARTMENT_SHORT_NAME_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_DEPARTMENT_SHORT_NAME_OVERRIDE:
        return {"GLDepartmentShortName": GL_DEPARTMENT_SHORT_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_GL_DEPARTMENT_SHORT_NAME", "").strip()
    if env_value:
        return {"GLDepartmentShortName": env_value}
...
class TestGlDepartmentGetByShortName(unittest.TestCase):
    ...
    def test_gl_department_get_by_short_name(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-department-short-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_DEPARTMENT_SHORT_NAME_OVERRIDE = args.field_value.strip()
