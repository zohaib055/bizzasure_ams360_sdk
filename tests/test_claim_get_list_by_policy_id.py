...
METHOD = 'claim_get_list_by_policy_id'
REQUEST_MODEL = 'ClaimGetListByPolicyIdRequest'
FIELD_NAMES = ['PolicyId']
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'claim_get_list_by_policy_id.json'
...
def _load_payload() -> dict:
    if POLICY_ID_OVERRIDE:
        return {"PolicyId": POLICY_ID_OVERRIDE}

    env_value = os.getenv("AMS360_POLICY_ID", "").strip()
    if env_value:
        return {"PolicyId": env_value}
...
class TestClaimGetListByPolicyId(unittest.TestCase):
    ...
    def test_claim_get_list_by_policy_id(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy-id", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        POLICY_ID_OVERRIDE = args.field_value.strip()
