...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'company_get_by_short_name.json'
LOG_PATH = ROOT_DIR / "log.txt"
COMPANY_SHORT_NAME_OVERRIDE = None
...
def _load_payload() -> dict:
    if COMPANY_SHORT_NAME_OVERRIDE:
        return {"CompanyShortName": COMPANY_SHORT_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_COMPANY_SHORT_NAME", "").strip()
    if env_value:
        return {"CompanyShortName": env_value}
...
class TestCompanyGetByShortName(unittest.TestCase):
    ...
    def test_company_get_by_short_name(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--company-short-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        COMPANY_SHORT_NAME_OVERRIDE = args.field_value.strip()
