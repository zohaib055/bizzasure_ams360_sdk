...
FIELD_NAMES = ['ClaimId']
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'claim_get_by_id.json'
LOG_PATH = ROOT_DIR / "log.txt"
CLAIM_ID_OVERRIDE = None
...
def _load_payload() -> dict:
    if CLAIM_ID_OVERRIDE:
        return {"ClaimId": CLAIM_ID_OVERRIDE}

    env_value = os.getenv("AMS360_CLAIM_ID", "").strip()
    if env_value:
        return {"ClaimId": env_value}
...
class TestClaimGetById(unittest.TestCase):
    ...
    def test_claim_get_by_id(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--claim-id", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        CLAIM_ID_OVERRIDE = args.field_value.strip()
