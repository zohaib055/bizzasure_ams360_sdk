...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_department_get_list_by_gl_branch_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_BRANCH_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_BRANCH_CODE_OVERRIDE:
        return {"GLBranchCode": GL_BRANCH_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_GL_BRANCH_CODE", "").strip()
    if env_value:
        return {"GLBranchCode": env_value}
...
class TestGlDepartmentGetListByGlBranchCode(unittest.TestCase):
    ...
    def test_gl_department_get_list_by_gl_branch_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-branch-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_BRANCH_CODE_OVERRIDE = args.field_value.strip()
