...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'vendor_get_list_by_last_name_prefix.json'
LOG_PATH = ROOT_DIR / "log.txt"
VENDOR_LAST_NAME_PREFIX_OVERRIDE = None
...
def _load_payload() -> dict:
    if VENDOR_LAST_NAME_PREFIX_OVERRIDE:
        return {"VendorLastNamePrefix": VENDOR_LAST_NAME_PREFIX_OVERRIDE}

    env_value = os.getenv("AMS360_VENDOR_LAST_NAME_PREFIX", "").strip()
    if env_value:
        return {"VendorLastNamePrefix": env_value}
...
class TestVendorGetListByLastNamePrefix(unittest.TestCase):
    ...
    def test_vendor_get_list_by_last_name_prefix(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vendor-last-name-prefix", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        VENDOR_LAST_NAME_PREFIX_OVERRIDE = args.field_value.strip()
