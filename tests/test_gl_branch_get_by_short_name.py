...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_branch_get_by_short_name.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_BRANCH_SHORT_NAME_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_BRANCH_SHORT_NAME_OVERRIDE:
        return {"GLBranchShortName": GL_BRANCH_SHORT_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_GL_BRANCH_SHORT_NAME", "").strip()
    if env_value:
        return {"GLBranchShortName": env_value}
...
class TestGlBranchGetByShortName(unittest.TestCase):
    ...
    def test_gl_branch_get_by_short_name(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-branch-short-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_BRANCH_SHORT_NAME_OVERRIDE = args.field_value.strip()
