...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_group_get_by_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_GROUP_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_GROUP_CODE_OVERRIDE:
        return {"GLGroupCode": GL_GROUP_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_GL_GROUP_CODE", "").strip()
    if env_value:
        return {"GLGroupCode": env_value}
...
class TestGlGroupGetByCode(unittest.TestCase):
    ...
    def test_gl_group_get_by_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-group-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_GROUP_CODE_OVERRIDE = args.field_value.strip()
