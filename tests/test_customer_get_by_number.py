...
DEMO_PAYLOAD_PATH = ROOT_DIR / "demos" / "customer_get_by_number.json"
FIELD_NAMES = ["CustomerNumber"]
LOG_PATH = ROOT_DIR / "log.txt"
CUSTOMER_NUMBER_OVERRIDE = None
...
def _load_payload() -> dict:
    if CUSTOMER_NUMBER_OVERRIDE:
        return {"CustomerNumber": CUSTOMER_NUMBER_OVERRIDE}

    env_customer_number = os.getenv("AMS360_CUSTOMER_NUMBER", "").strip()
    if env_customer_number:
        return {"CustomerNumber": env_customer_number}
...
class TestCustomerGetByNumber(unittest.TestCase):
    ...
    def test_customer_get_by_number(self) -> None:
        ...
        if "CustomerNumber" not in payload:
            self.fail("CustomerNumber is required in the payload.")

        client = self._get_client()
        gen = Generated(client)
        request = models.CustomerGetByNumberRequest(**payload)
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--customer-number", dest="customer_number", default=None)
    args, remaining = parser.parse_known_args()
    if args.customer_number:
        CUSTOMER_NUMBER_OVERRIDE = args.customer_number.strip()
