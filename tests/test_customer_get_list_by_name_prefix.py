...
DEMO_PAYLOAD_PATH = ROOT_DIR / "demos" / "customer_get_list_by_name_prefix.json"
...
def _load_payload() -> dict:
    ...
    if env_json:
        ...
        if payload_file:
            payload = _load_json(Path(payload_file))
        elif DEMO_PAYLOAD_PATH.exists():
            payload = _load_json(DEMO_PAYLOAD_PATH)

    if NAME_PREFIX_OVERRIDE:
        payload["NamePrefix"] = NAME_PREFIX_OVERRIDE
    elif not payload.get("NamePrefix"):
        env_prefix = os.getenv("AMS360_CUSTOMER_NAME_PREFIX", "").strip()
        if env_prefix:
            payload["NamePrefix"] = env_prefix
        elif not payload:
            payload["NamePrefix"] = "A"
...
class TestCustomerGetListByNamePrefix(unittest.TestCase):
    ...
    def test_customer_get_list_by_name_prefix(self) -> None:
        ...
        if "NamePrefix" not in payload:
            self.fail("NamePrefix is required in the payload.")

        client = self._get_client()
        gen = Generated(client)
        request = models.CustomerGetListByNamePrefixRequest(**payload)
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name-prefix", dest="name_prefix", default=None)
    args, remaining = parser.parse_known_args()
    if args.name_prefix:
        NAME_PREFIX_OVERRIDE = args.name_prefix.strip()
