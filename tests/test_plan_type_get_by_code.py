...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'plan_type_get_by_code.json'
...
def _load_payload() -> dict:
    if PLAN_CODE_OVERRIDE:
        return {"PlanCode": PLAN_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_PLAN_CODE", "").strip()
    if env_value:
        return {"PlanCode": env_value}
...
class TestPlanTypeGetByCode(unittest.TestCase):
    ...
    def test_plan_type_get_by_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plan-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        PLAN_CODE_OVERRIDE = args.field_value.strip()
