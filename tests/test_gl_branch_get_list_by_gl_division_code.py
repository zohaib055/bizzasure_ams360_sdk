...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_branch_get_list_by_gl_division_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_DIVISION_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_DIVISION_CODE_OVERRIDE:
        return {"GLDivisionCode": GL_DIVISION_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_GL_DIVISION_CODE", "").strip()
    if env_value:
        return {"GLDivisionCode": env_value}
...
class TestGlBranchGetListByGlDivisionCode(unittest.TestCase):
    ...
    def test_gl_branch_get_list_by_gl_division_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-division-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_DIVISION_CODE_OVERRIDE = args.field_value.strip()
