...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'broker_get_by_short_name.json'
LOG_PATH = ROOT_DIR / "log.txt"
BROKER_SHORT_NAME_OVERRIDE = None
...
def _load_payload() -> dict:
    if BROKER_SHORT_NAME_OVERRIDE:
        return {"BrokerShortName": BROKER_SHORT_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_BROKER_SHORT_NAME", "").strip()
    if env_value:
        return {"BrokerShortName": env_value}
...
class TestBrokerGetByShortName(unittest.TestCase):
    ...
    def test_broker_get_by_short_name(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--broker-short-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        BROKER_SHORT_NAME_OVERRIDE = args.field_value.strip()
