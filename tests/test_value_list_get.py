...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'value_list_get.json'
...
def _load_payload() -> dict:
    if LIST_NAME_OVERRIDE:
        return {"ListName": LIST_NAME_OVERRIDE}

    env_value = os.getenv("AMS360_LIST_NAME", "").strip()
    if env_value:
        return {"ListName": env_value}
...
class TestValueListGet(unittest.TestCase):
    ...
    def test_value_list_get(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--list-name", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        LIST_NAME_OVERRIDE = args.field_value.strip()
