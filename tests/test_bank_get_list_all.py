...
class TestBankGetListAll(unittest.TestCase):
    def _get_client(self) -> AMS360Client:
        ...
        try:
            force_login = settings.has_credentials()
            if force_login:
                settings.ticket = None
    ...
    def test_bank_get_list_all(self) -> None:
        client = self._get_client()
        gen = Generated(client)
        soap_path = ROOT_DIR / "tests" / "bank_get_list_all.soap.log"

        try:
            result = getattr(gen, f"{METHOD}_json")()
        except AMS360SoapError as exc:
            soap = client.last_soap()
            soap_text = (
                "--- sent ---\n"
                f"{soap.get('sent', '')}\n"
                "--- received ---\n"
            ...
            soap_path.write_text(soap_text, encoding="utf-8")
            LOGGER.error("SOAP error: %s (saved SOAP to %s)", exc, soap_path)
            self.skipTest(f"AMS360 SOAP error: {exc}")
        else:
            soap = client.last_soap()
            if soap.get("sent") or soap.get("received"):
                soap_text = (
                    "--- sent ---\n"
                    f"{soap.get('sent', '')}\n"
                    "--- received ---\n"
                ...
                soap_path.write_text(soap_text, encoding="utf-8")
                LOGGER.info("Saved SOAP to %s", soap_path)
