...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'bank_get_by_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
BANK_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if BANK_CODE_OVERRIDE:
        return {"BankCode": BANK_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_BANK_CODE", "").strip()
    if env_value:
        return {"BankCode": env_value}
...
class TestBankGetByCode(unittest.TestCase):
    ...
    def test_bank_get_by_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bank-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        BANK_CODE_OVERRIDE = args.field_value.strip()
