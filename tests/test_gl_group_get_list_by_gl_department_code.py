...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'gl_group_get_list_by_gl_department_code.json'
LOG_PATH = ROOT_DIR / "log.txt"
GL_DEPARTMENT_CODE_OVERRIDE = None
...
def _load_payload() -> dict:
    if GL_DEPARTMENT_CODE_OVERRIDE:
        return {"GLDepartmentCode": GL_DEPARTMENT_CODE_OVERRIDE}

    env_value = os.getenv("AMS360_GL_DEPARTMENT_CODE", "").strip()
    if env_value:
        return {"GLDepartmentCode": env_value}
...
class TestGlGroupGetListByGlDepartmentCode(unittest.TestCase):
    ...
    def test_gl_group_get_list_by_gl_department_code(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gl-department-code", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        GL_DEPARTMENT_CODE_OVERRIDE = args.field_value.strip()
