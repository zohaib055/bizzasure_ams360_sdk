...
PAYLOAD_PATH = ROOT_DIR / 'demos' / 'company_get_list_by_type.json'
LOG_PATH = ROOT_DIR / "log.txt"
COMPANY_TYPES_OVERRIDE = None
...
def _load_payload() -> dict:
    if COMPANY_TYPES_OVERRIDE:
        return {"CompanyTypes": COMPANY_TYPES_OVERRIDE}

    env_value = os.getenv("AMS360_COMPANY_TYPES", "").strip()
    if env_value:
        return {"CompanyTypes": env_value}
...
class TestCompanyGetListByType(unittest.TestCase):
    ...
    def test_company_get_list_by_type(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--company-types", dest="field_value", default=None)
    args, remaining = parser.parse_known_args()
    if args.field_value:
        COMPANY_TYPES_OVERRIDE = args.field_value.strip()
